...
```{r}
ggplot(tents,aes(x=brand))+geom_bar(aes(fill = brand))
```
...
```{r}
ggplot(tents,aes(x=height,y=weight, color = bestuse))+geom_point()
```
...
wcratio<- tents$weight/capacity

mutate(tents,"weight_to_capacity"= wcratio) %>% 
