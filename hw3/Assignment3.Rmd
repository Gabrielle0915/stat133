...
Sandwich <- function(n){
  ...
  while(n %/% divider >= 1){
    digits = digits + 1
    intList[digits] = n %% divider
    n = (n - intList[digits])/10
  }
  digits = digits + 1
  intList[digits] = (n*10) %/% divider
  intList <- intList[1:digits]
  i = 1  
  while (i <= digits - 2) {
...
Sandwich2 <- function(n){
  if(n < 100){
    ...
    intString = as.character(n)
    return(str_detect(intString, "([0-9])([0-9])\\1"))
  }
}
```
...
BigMac <- function(n){
  if(n < 1000){
    ...
    intString = as.character(n)
    return(str_detect(intString, "([0-9])([0-9])([0-9])\\1"))
  }
}
```
...
MSandwich <- function(n,m){
    intString = as.character(n)
    patternSandwich = str_c("([0-9])([0-9]){", as.character(m), "}\\1")
    return(str_detect(intString, patternSandwich))
}
```
...
states[(str_count(states, "[a-z]") %% 2) == 1] <- toupper(states[((str_count(states, "[a-z]") %% 2) == 1)])
numberRange = sort(unique(str_count(states, "[a-zA-Z]")))
states_list <- lapply(numberRange, function(x) states[(str_count(states, "[a-zA-Z]")) == x])
ListName = str_c("charcount", as.character(numberRange))
names(states_list) <- ListName
states_list
```
...
```{r}
# Your code here
Filelist = str_c("babynames/", list.files("babynames"));
FileNumber = length(Filelist)
Filelist <- Filelist[2:FileNumber]
i = 1
TotalBeforethisYear = 2000
babyNames <- read_csv(file = Filelist[i], col_names = c("name", "sex", "n", "year"), col_types = "cci")
babyNames$year = 1880
i = i + 1
while(i < FileNumber){
  newFile <- read_csv(file = Filelist[i], col_names = c("name", "sex", "n", "year"), col_types = "cci")
  babyNames <- rbind(babyNames,newFile)
  numInThisYear = nrow(newFile)
  babyNames$year[(TotalBeforethisYear+1):(TotalBeforethisYear+numInThisYear)] = 1879+i
  TotalBeforethisYear = TotalBeforethisYear + numInThisYear
  i = i + 1
}
...
# Your code here
years = c(2006:2015)
femalePercentage <- rep(0,10)
malePercentage <- rep(0,10)
totalPercentage <- rep(0,10)

for (i in 2006:2015){
  ThisYear1000F<-  babyNames %>%
    filter(sex == "F") %>%
    filter(year == i) %>%
    arrange(desc(n)) %>%
    head(1000) %>%
    summarise(ThisYear1000 = sum(n))
  ThisYearTotalF <- babyNames %>%
    filter(sex == "F") %>%
    filter(year == i) %>%
    summarise(ThisYearTotal = sum(n))
  femalePercentage[i-2005] = round(ThisYear1000F[[1]] / ThisYearTotalF[[1]],4)
  
  ThisYear1000M<-  babyNames %>%
    filter(sex == "M") %>%
    filter(year == i) %>%
    arrange(desc(n)) %>%
    head(1000) %>%
    summarise(ThisYear1000 = sum(n))
  ThisYearTotalM <- babyNames %>%
    filter(sex == "M") %>%
    filter(year == i) %>%
    summarise(ThisYearTotal = sum(n))
  malePercentage[i-2005] = round(ThisYear1000M[[1]] / ThisYearTotalM[[1]],4)
  
  ThisYear1000<-  babyNames %>%
    filter(year == i) %>%
    group_by(name) %>%
    summarise(count = sum(n)) %>%
    arrange(desc(count)) %>%
    head(1000) %>%
    summarise(ThisYear1000 = sum(count))
  ThisYearTotal <- babyNames %>%
    filter(year == i) %>%
    summarise(ThisYearTotal = sum(n))
  totalPercentage[i-2005] = round(ThisYear1000[[1]] / ThisYearTotal[[1]], 4)
  
}

nameperc <- data.frame(year = c(2006:2015), male = malePercentage, female = femalePercentage, total = totalPercentage)
nameperc    
