...
```{r}
# Your code
dfStatisticians <- as.data.frame(pioneers_df)
colnames(dfStatisticians) <- c("name", "byear") 
dfStatisticians
```
...
```{r}
# Your code
dfStatisticians$name <- as.character(dfStatisticians$name)
```
...
```{r}
# Your code
vapply(1:10, function(x) split_sleep[[x]][2,1] - split_sleep[[x]][1,1], double(1))
```
...
# Your code
add_length <- iris %>%
  mutate(length_sum = Petal.Length + Sepal.Length)
add_length
```
...
nonsense_filter <- iris %>%
  filter(Sepal.Length+Sepal.Width<8 & str_count(Species, "[aeiou]") == 4)
nonsense_filter
```
