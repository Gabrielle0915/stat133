...
```{r}
# your code
FlipTilHeads <- function(probability = 0.5){
  times = 10000
  sampleResults <- sample(c(0,1), times, TRUE, c(1 - probability, probability))
  i = 1
  while(sampleResults[i] == 0){
    i = i + 1
  }
  return(i)
}
...
Hailstone <- function(x){
  HailstoneSequence <- c(1:1000000)
  iter = 1;
  while(x != 1){
    HailstoneSequence[iter] = x
    if(x %% 2 == 0){
      x = x/2
    }
    else{
  ...
  HailstoneSequence[iter] = 1
  HailstoneSequence = HailstoneSequence[1:iter]
  result <- list(iterationOfHailstone = iter-1, sequenceOfHailstone = HailstoneSequence)
  return(result)
