...
```{r}
data <- read.csv("2014q4_trips.csv")
stations <- read.csv("dc_stations.csv")
```
...
data <- data %>%
  mutate(timeString = hms(str_extract(sdate, "[0-9]{2}:[0-9]{2}:[0-9]{2}"))) %>%
  mutate(numericTime = hour(timeString) + minute(timeString)/60) %>%
  mutate(dateString = ymd(str_extract(sdate, "[0-9]{4}-[0-9]{2}-[0-9]{2}"))) %>%
  mutate(numericDate = wday(dateString)) %>%
  mutate(week = ifelse(numericDate >6 | numericDate < 2, "Weekend", "Weekday"))

ggplot(data, aes(x = numericTime, y = ..density..)) + geom_density(aes(fill = client), alpha = 0.3, color = NA) + facet_wrap(~week) + ggtitle("Temporal Trends in Bike Usage") + xlab("Time of Day") + ylab("Density")
...
```{r, fig.width=9, fig.asp=.5}
crossingStation <- crossing(stations, stations)
names(crossingStation) <- c("sstation", "slat", "slong", "1", "2", "estation", "elat", "elong", "3", "4")
source("haversine.R")
crossingStation <- crossingStation %>%
  select(sstation, slat, slong, estation, elat, elong) %>%
  inner_join(data) %>%
  mutate(Distance = haversine(slat, slong, elat, elong))

ggplot(crossingStation, aes(x = Distance, y = ..density..)) + geom_density(aes(fill = client), alpha = 0.5, color = NA) + facet_wrap(~week) + ggtitle("Distance between Start and End Station") + xlab("Distance(km)") + ylab("Density") + scale_x_continuous(limits = c(0.0, 10.0), breaks = seq(0.0, 10.0, by = 2.5))
