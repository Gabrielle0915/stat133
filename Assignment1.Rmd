...
```{r}
longest_flights <- flights$air_time[(order(flights$air_time, decreasing = TRUE)[1:10])]
#head(sort(flights$air_time,decreasing = TRUE),10)
longest_flights
...
y <- 1:12
y[1] <- mean(flights$dep_delay[flights[["month"]] == 1], na.rm = TRUE)
y[2] <- mean(flights$dep_delay[flights[["month"]] == 2], na.rm = TRUE)
y[3] <- mean(flights$dep_delay[flights[["month"]] == 3], na.rm = TRUE)
y[4] <- mean(flights$dep_delay[flights[["month"]] == 4], na.rm = TRUE)
y[5] <- mean(flights$dep_delay[flights[["month"]] == 5], na.rm = TRUE)
y[6] <- mean(flights$dep_delay[flights[["month"]] == 6], na.rm = TRUE)
y[7] <- mean(flights$dep_delay[flights[["month"]] == 7], na.rm = TRUE)
y[8] <- mean(flights$dep_delay[flights[["month"]] == 8], na.rm = TRUE)
y[9] <- mean(flights$dep_delay[flights[["month"]] == 9], na.rm = TRUE)
y[10] <- mean(flights$dep_delay[flights[["month"]] == 10], na.rm = TRUE)
y[11] <- mean(flights$dep_delay[flights[["month"]] == 11], na.rm = TRUE)
y[12] <- mean(flights$dep_delay[flights[["month"]] == 12], na.rm = TRUE)
label = 1:12
month_delay <- label[order(y,decreasing = TRUE)]
month_delay
...
```{r}
num_planes_aa <- length(unique(flights$tailnum[flights[["carrier"]] == "AA"]))
num_planes_aa
```

8. How many flights originating from JFK flew more than 1000 miles? __jfk_1000mi__
```{r}
jfk_1000mi <- nrow(flights[flights$origin == "JFK" & flights$distance > 1000,])
jfk_1000mi
...
```{r}
lga_dfw <- flights[flights$origin == "LGA" & flights$dest == "DFW", ]
num_lga_dfw <- NROW(table(lga_dfw$carrier))
num_lga_dfw
...
most_flights_jfk <- sort(table(JFKflights$carrier),decreasing = TRUE)[1]
most_flights_jfk
```
